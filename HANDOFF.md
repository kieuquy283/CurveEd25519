...
## TASK-AUTH-UI-REDESIGN-002 - Split-screen crypto auth layout (v2)

Status: DONE (frontend build verified)

Scope:
- UI-only auth redesign for login/register/verify/forgot/reset.
- Preserved existing auth business logic, handlers, store state, and API calls.
- No backend/chat/crypto service changes.

What changed:
- Added `ui/src/components/auth/AuthShell.tsx`
  - Full viewport auth shell with split-screen responsive layout.
  - Left hero + right form container.
  - Subtle animated neon glow background effects.
- Added `ui/src/components/auth/AuthCard.tsx`
  - Reusable glassmorphism auth card wrapper.
- Updated `ui/src/components/auth/AuthHero.tsx`
  - New branding/hero content:
    - Headline: `Private conversations. Proven cryptography.`
    - Subtitle about encrypted messaging + signatures.
    - Feature bullets and trust badge.
    - CSS-based secure illustration (no external images).
- Rebuilt `ui/src/components/auth/AuthScreen.tsx` presentation
  - Kept mode model: `login | register | verify | forgot | reset`.
  - Kept all existing handlers:
    - `onLogin`, `onRegister`, `onVerify`, `onResendVerification`, `onRequestReset`, `onResetPassword`.
  - Kept `dev_code` display.
  - Kept loading/error/message rendering.
  - Added segmented tabs (`Log in` / `Sign up`) and modern input/button styling.
  - Added password visibility toggles.
  - Fixed Vietnamese strings to valid UTF-8.
- Updated `ui/src/app/globals.css`
  - Added `auth-glow-float` animation and `.auth-glow-orb` utility for subtle background motion.

Unaffected logic:
- `ui/src/app/page.tsx` auth-vs-chat conditional remains unchanged.
- `useAuthStore` shape/behavior unchanged.
- `services/auth.ts` endpoints unchanged.

Checks run:
- `cd ui && npm run build` OK

Manual verification still required in browser:
1. Logged-out screen renders split hero + auth card.
2. Login flow works and enters main app.
3. Register flow works.
4. Verify email + resend code works (including dev code display when provided).
5. Forgot/reset password flows work.
6. Mobile layout remains usable.

## 2026-05-13 - OSV Follow-up: Next.js Upgrade to 16.2.6

Status: DONE (upgrade validated, no frontend code fixes required)

Scope:
- Applied in `ui/` only.
- Did not change backend, crypto, chat transport, Supabase, Brevo, or WebSocket logic.

Version updates:
- `ui/package.json`
  - `next`: `15.5.18` -> `16.2.6` (exact version)
- `ui/package-lock.json`
  - Regenerated by `npm install`.
- `ui/tsconfig.json`
  - Auto-updated by Next.js build tooling:
    - `compilerOptions.jsx` set to `react-jsx`.
...
- `npm run build`
  - Result: success on Next.js `16.2.6`.
  - Build summary:
    - Compiled successfully.
    - TypeScript checks completed.
    - Static pages generated.

Compatibility notes:
- No build/type issues requiring source code patches were introduced by Next 16 in this run.
- Upgrade path to `next@16.2.6` is currently viable for this repository state.

## 2026-05-13 - OSV Follow-up: Remaining MEDIUM Warning After Next 16.2.6

Status: DONE (frontend dependency verification and build validated)

Scope:
- Applied in `ui/` only.
- No backend, crypto, chat, Supabase, Brevo, or WebSocket logic changed.

Verified dependency versions in `ui/package.json` and `ui/package-lock.json`:
- `next`: `16.2.6`
- `react`: `19.2.4`
- `react-dom`: `19.2.4`
- `react-server-dom-webpack`: `19.1.7`
- `react-server-dom-turbopack`: `19.1.7`
...
- `npm audit --omit=dev`
  - Result: `found 0 vulnerabilities`
- `npm install`
  - Result: success (`removed 4 packages, and audited 739 packages in 5s`)
  - Note: full-tree audit summary from install reported `2 moderate severity vulnerabilities` (this includes dev dependencies and differs from `--omit=dev` scope).
- `npm run build`
  - Result: success on `Next.js 16.2.6 (Turbopack)`.
  - Build completed with static routes generated.
- `npm audit --omit=dev` (re-run after install)
  - Result: `found 0 vulnerabilities`

Conclusion for VS Code OSV warning:
- If VS Code `rhtpa(osv-github)` still shows `next@16.2.6` MEDIUM after the above, it is likely an OSV scanner cache/stale advisory mapping or false positive for this lockfile state.
- Current production dependency audit (`npm audit --omit=dev`) is clean.

## 2026-05-13 - Conversation UI Redesign Aligned With Auth Visual Style

Status: DONE (UI/UX restyle, logic preserved)

Scope:
- Frontend conversation/chat presentation only in `ui/`.
- No backend, crypto protocol, websocket protocol, auth flow, Supabase schema, or API payload changes.

Conversation UI redesign summary:
- Applied premium dark crypto visual system to chat area:
  - Deep navy/black shell background with violet/blue/fuchsia glow orbs.
  - Glassmorphism cards/panels (`rounded-[2rem]`, subtle borders, blur, glow shadows).
  - Softer modern hierarchy for sidebar, message thread, composer, and info panel.
- Upgraded to a polished 3-panel experience:
  - Left sidebar restyled with brand block, search field, visual tabs, rounded conversation cards, active glow state, user/footer privacy card.
  - Center panel restyled with secure header, encryption notice, softer incoming/outgoing bubbles, polished action buttons.
  - Right info panel restyled to glass card with profile summary, quick actions, section cards, and mobile overlay drawer.
- Label/wording cleanup:
  - Corrected key Vietnamese strings and secure-state labels used in conversation UI (e.g. “Tin nhắn được mã hóa đầu cuối”, “Đã kết nối an toàn”, “Đang kết nối...”, “Mất kết nối”, “Tệp đã chia sẻ”, “Biệt danh”, “Thông tin cuộc trò chuyện”).
...
- `ui/src/components/ConversationItem.tsx`
- `ui/src/components/MessageList.tsx`
- `ui/src/components/MessageBubble.tsx`
- `ui/src/components/MessageComposer.tsx`
- `ui/src/components/conversation/ConversationInfoPanel.tsx`
- `ui/src/components/attachments/AttachmentBubble.tsx`
- `ui/src/components/attachments/AttachmentPreview.tsx`
- `ui/src/components/TypingIndicator.tsx`

Logic-preservation notes:
- Kept existing handlers/stores/API calls/data models in place:
  - Conversation load/select/unread clear.
  - Message persistence and list loading.
  - WebSocket connection status behavior and reconnect action.
  - Encrypted text/file sending flow.
  - Signed file sign/verify flow and signed file actions.
  - Crypto trace open/decrypt trace behavior.
  - Notification click-to-conversation behavior.
  - Info panel toggle behavior and mobile close overlay.
  - Nickname edit persistence + metadata patch path.
...
- Build + type safety checks: passed.
- UI interaction sanity (code-level verification):
  - Conversation select/highlight path preserved.
  - Info panel open/close toggle preserved.
  - Conversation-scoped attachment listing and search logic preserved.
  - Composer send/attach/sign/verify handlers preserved.
  - Notification deep-link conversation open flow preserved.
