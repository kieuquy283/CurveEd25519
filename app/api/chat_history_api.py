...
def list_messages(conversation_id: str, user: str = Query(...), limit: int = Query(50), before: str | None = Query(None)):
    user_norm = _norm(user)
    ...
    storage.create_notification(
        {
            ...
            "type": "message",
            "title": sender,
            "body": (body.plaintext_preview or "").strip()[:120] or "Tin nhắn mới",
            "data": {
                "peerEmail": sender,
